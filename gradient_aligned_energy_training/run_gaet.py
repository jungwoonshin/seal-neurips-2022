"""
Train with Gradient-Aligned Energy Training (GAET) on bibtex for 40 epochs.

Uses MLME as the energy architecture and replaces NCE ranking with gradient
alignment: the energy function is trained so its gradient aligns with the
gradient of soft F1, rather than to rank configurations.

    L_E = -cos(∇_ỹ E_Θ(x, ỹ), ∇_ỹ S(ỹ, y*))

Results are logged to output/gaet_run/epoch_results.txt.
"""
...
SERIALIZATION_DIR = os.path.join("output", "gaet_run")
...
def write_config():
    ...
    config_path = os.path.join(SERIALIZATION_DIR, "gaet_config.json")
...
def run_training(config_path):
    ...
    cmd = [
        ...
        "--include-package", "gradient_aligned_energy_training",
        "-f",
    ]
    print(f"Running: {' '.join(cmd)}")
    # Run from the project root (parent of this script's directory)
...
def collect_epoch_results():
    ...
    with open(output_path, "w", encoding="utf-8") as out:
        out.write("=" * 100 + "\n")
        out.write("GAET (Gradient-Aligned Energy Training) -- Bibtex Training Results (40 epochs)\n")
        out.write("=" * 100 + "\n\n")
